...
RUN cd /opt && curl -sL https://github.com/oracle/graal/releases/download/vm-1.0.0-rc10/graalvm-ce-1.0.0-rc10-linux-amd64.tar.gz | tar -xzvf -
ADD project.clj .
RUN lein deps
ADD src src
ADD reflection.json reflection.json
ENV GRAALVM_HOME /opt/graalvm-ce-1.0.0-rc10/bin
